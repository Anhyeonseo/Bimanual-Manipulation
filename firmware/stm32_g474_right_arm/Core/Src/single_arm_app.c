...
)
{
  ...
  int calibration_line_length = snprintf(
      status_line,
      sizeof(status_line),
      "CALIBRATION_LOCK ID=%u RAW_DELTA=+%u DURATION=%luMS NORMAL_MOTION=LOCKED\r\n",
      (unsigned int)RIGHT_ARM_CALIBRATION_SERVO_ID,
      (unsigned int)RIGHT_ARM_CALIBRATION_DELTA_RAW,
      (unsigned long)RIGHT_ARM_CALIBRATION_DURATION_MS
  );

  if ((calibration_line_length > 0) &&
      (calibration_line_length < (int)sizeof(status_line)))
  {
      (void)HAL_UART_Transmit(
          app_host_uart,
          (uint8_t *)status_line,
          (uint16_t)calibration_line_length,
          100U
      );
  }
#endif
...
void SingleArmApp_Process(void)
{
    ...
    BinaryControl_Service();
	  ...
	  {
#if RIGHT_ARM_CALIBRATION_MODE
	      static const uint8_t calibration_binary_lock[] =
	          "BINARY_MODE_LOCKED_DURING_CALIBRATION\r\n";

	      (void)HAL_UART_Transmit(
	          app_host_uart,
	          (uint8_t *)calibration_binary_lock,
	          sizeof(calibration_binary_lock) - 1U,
	          100U
	      );
#else
	  ...
	  else if ((rx_byte == 'D') || (rx_byte == 'd'))
	  {
	      ...
	      HAL_StatusTypeDef disable_status = Servo_DisableTorqueAll();
	      static const uint8_t disabled[] =
	          "TORQUE_DISABLED_ALL_VERIFIED STOP_LATCHED_RESET_TO_CONTINUE\r\n";
	      static const uint8_t disable_failed[] =
	          "TORQUE_DISABLE_VERIFY_FAILED REMOVE_12V_POWER\r\n";
	      const uint8_t *disable_message =
	          (disable_status == HAL_OK) ? disabled : disable_failed;
	      uint16_t disable_message_length =
	          (disable_status == HAL_OK)
	              ? (uint16_t)(sizeof(disabled) - 1U)
	              : (uint16_t)(sizeof(disable_failed) - 1U);

	      (void)HAL_UART_Transmit(
	          app_host_uart,
	          (uint8_t *)disable_message,
	          disable_message_length,
	          100U
	      );
	  }
	      ...
	      else if (absolute_pose_active != 0U)
	      {
    	      ...
    	      else
    	      {
    	          ...
    	          for (uint8_t joint_index = 0U;
    	               ...
    	               joint_index++)
    	          {
    	              uint8_t selection_allowed =
#if RIGHT_ARM_CALIBRATION_MODE
    	                  (requested_id == RIGHT_ARM_CALIBRATION_SERVO_ID)
    	                      ? 1U : 0U;
#else
    	                  servo_joints[joint_index].motion_enabled;
#endif

    	              if ((servo_joints[joint_index].id ==
    	                      requested_id) &&
    	                  (selection_allowed != 0U))
    	              {
    	                  selection_found = 1U;
    	                  system_ready = 0U;

    	                  uint16_t configured_position = 0U;
	                  uint16_t requested_torque_limit =
	                      servo_joints[joint_index].torque_limit;
	                  uint8_t requested_p_gain =
	                      servo_joints[joint_index].p_gain;

#if RIGHT_ARM_CALIBRATION_MODE
	                  requested_torque_limit =
	                      RIGHT_ARM_CALIBRATION_TORQUE_LIMIT_RAW;
	                  requested_p_gain = RIGHT_ARM_CALIBRATION_P_GAIN;
#endif
	                  ...
	                  if (Servo_ConfigureForTrajectory(
    	                      ...
    	                      ) == HAL_OK)
    	                  {
    	                      ...
    	                      active_test_delta =
#if RIGHT_ARM_CALIBRATION_MODE
    	                          RIGHT_ARM_CALIBRATION_DELTA_RAW;
#else
    	                      ...
    	                      active_duration_ms =
#if RIGHT_ARM_CALIBRATION_MODE
    	                          RIGHT_ARM_CALIBRATION_DURATION_MS;
#else
	  ...
	  else if ((rx_byte == 'J') || (rx_byte == 'j'))
	  {
	      ...
	      {
	          static const uint8_t calibration_lock[] =
	              "J_LOCKED_CALIBRATION_SINGLE_JOINT_ONLY\r\n";

	          (void)HAL_UART_Transmit(
	              app_host_uart,
	              (uint8_t *)calibration_lock,
	              sizeof(calibration_lock) - 1U,
	              100U
	          );
	      }
	      else if (synchronized_pose_active != 0U)
	      {
	          static const uint8_t sync_return_first[] =
	  ...
	  else if ((rx_byte == 'A') || (rx_byte == 'a'))
	  {
	      ...
	      {
	          static const uint8_t calibration_lock[] =
	              "A_LOCKED_CALIBRATION_SINGLE_JOINT_ONLY\r\n";
	  ...
	  else if ((rx_byte == 'B') || (rx_byte == 'b'))
	  {
	      ...
	      {
	          static const uint8_t calibration_lock[] =
	              "B_LOCKED_CALIBRATION_SINGLE_JOINT_ONLY\r\n";
...
#if RIGHT_ARM_CALIBRATION_MODE
	                  (servo_joints[joint_index].id ==
	                   RIGHT_ARM_CALIBRATION_SERVO_ID)
	                      ? "CALIBRATION_ONLY"
	                      : "LOCKED";
#else
    	  ...
    	  else if ((rx_byte == 'M') || (rx_byte == 'm'))
          {
              ...
              else
              {
                  ...
                  {
#if RIGHT_ARM_CALIBRATION_MODE
	                  uint16_t calibration_home_offset =
	                      (start_position >= active_home_position)
	                          ? (uint16_t)(start_position - active_home_position)
	                          : (uint16_t)(active_home_position - start_position);

	                  if (calibration_home_offset >
	                      RIGHT_ARM_CALIBRATION_MAX_HOME_OFFSET_RAW)
	                  {
	                      int line_length = snprintf(
	                          status_line,
	                          sizeof(status_line),
	                          "CALIBRATION_START_REJECTED ID=%u POS=%u HOME=%u OFFSET=%u LIMIT=%u\r\n",
	                          (unsigned int)active_servo_id,
	                          (unsigned int)start_position,
	                          (unsigned int)active_home_position,
	                          (unsigned int)calibration_home_offset,
	                          (unsigned int)RIGHT_ARM_CALIBRATION_MAX_HOME_OFFSET_RAW
	                      );
                 	  ...
                 	  int32_t calculated_target =
	                      ...
	                      ((int32_t)RIGHT_ARM_CALIBRATION_DIRECTION *
	      ...
	      {
	          static const uint8_t calibration_lock[] =
	              "H_LOCKED_CALIBRATION_USE_M_THEN_R_ONLY\r\n";
